...
void ledInit( ){
	...
	unsigned int val_temp, val_temp2; 	
	/*-----------------------------------------------------------------------------
	...
	HWREG(SOC_CONTROL_REGS+CM_conf_lcd_data4) |= 7;
	...
	val_temp2 = HWREG(SOC_GPIO_2_REGS+GPIO_OE);
	val_temp2 &= ~(1<<6);
	val_temp2 &= ~(1<<10);
	HWREG(SOC_GPIO_2_REGS+GPIO_OE) = val_temp2;
